...
Диаграмма вариантов использования определяет три роли действующих лиц (акторов) и десять прецедентов:
...
Логическая архитектура приложения построена с использованием многоуровневого подхода, характерного для корпоративных систем на платформе Spring. Она включает шесть слоёв:
...
+ *UC4: Управление персональной информацией* — редактирование профиля с валидацией и частичным обновлением.

+ *UC3: Создание публикации* — создание поста с валидацией контента и сохранением в БД.

+ *UC3-2a: Загрузка и валидация медиафайлов* — циклическая обработка файлов с ветвлением по результатам валидации.
...
#figure(image("images/04-detailed-design/uc3-1.png", width: 90%), caption: [Диаграмма последовательности UC3-1: создание публикации]) <ui-post>
...
Паттерн *Builder* применяется для создания DTO и запросов с множеством опциональных полей с помощью библиотеки Lombok с использованием аннотации `@Builder`, реализующей паттерн GoF @gamma1995design. Паттерн *Decorator* используется для кеширования, ограничения частоты запросов и сбора метрик для портов через декорирование интерфейсов (`CachedUserQueryPort`).

Паттерн *Chain of Responsibility* реализован в виде цепочки обработчиков для проверки корректности (валидации) комментариев и постов (`LengthValidator` → `SpamValidator` → `ToxicityValidator`). Паттерн *Adapter* применяется для интеграции внешних систем (Matrix Federation, S3) через адаптеры (`MatrixFederationAdapter`, `MediaStorageAdapter`).
