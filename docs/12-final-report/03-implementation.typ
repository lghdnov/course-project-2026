...
Основные экраны: список чатов, комната, лента публикаций, редактор постов, комментарии, профиль пользователя, настройки.
...
- *CORS* — настроен строгий CORS-фильтр с явным перечислением разрешённых origin'ов в рабочей среде (production).
...
1. *Unit-тесты контроллеров* — через `@WebMvcTest` с заменой сервисов mock-объектами.
