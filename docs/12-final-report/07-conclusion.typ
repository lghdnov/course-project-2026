...
2. *Проектная часть* — разработана модель требований (прецеденты (use case), спецификация прецедентов, глоссарий), модель предметной области (Domain Model, бизнес-правила), архитектурный каркас на основе PCMEF с гексагональными принципами, спроектирована реляционная база данных (нормализация до 3НФ, DDL-скрипты), выполнено детальное проектирование (диаграммы последовательности, классов проектирования, паттерны GoF).

3. *Реализационная часть* — реализован бэкенд на Java Spring Boot с соблюдением слоистой архитектуры (Entity, Foundation, Mediator, Control), пользовательский интерфейс на React с интеграцией через REST API, система аутентификации через Matrix OpenID + JWT, а также документированное REST API (OpenAPI 3.1).
...
6. *Управление проектом* — разработана иерархическая структура работ (WBS), диаграмма Ганта, оценка трудозатрат по модели COCOMO, реестр рисков с планами митигации.
...
1. *Микросервисная декомпозиция* — выделение сервисов аутентификации, публикаций, уведомлений в отдельные развёртываемые модули (deployable units) @newman2021building.
