...
Предметная область охватывает функциональность социальных сетей и мессенджеров, интегрированных с протоколом Matrix @matrix-spec. Основные понятия:
...
Управление выполняется посредством политик безопасности и бизнес-правил. Механизмами выступают приложение на базе фреймворка Spring Boot, база данных PostgreSQL и веб-интерфейс на базе библиотеки React.
...
#figure(
  table(
    ...
    [Разрабатываемая система], [Федеративная (Matrix)], [Профили, посты, комментарии, алиасы, сообщества], [Да],
  ),
  caption: [Сравнительный анализ аналогов и конкурентов],
)

Главное конкурентное преимущество разрабатываемой системы — глубокая интеграция социальных функций (профили, публикации, комментарии) непосредственно в экосистему Matrix, что позволяет использовать существующую федеративную инфраструктуру без необходимости создания отдельной платформы.

Проведённый анализ показывает, что ни один из существующих продуктов на базе Matrix не предоставляет полноценных социальных функций в единой экосистеме. Это обусловливает необходимость разработки предлагаемой системы.

== Обоснование необходимости разработки

Разработка системы обоснована следующими соображениями:
