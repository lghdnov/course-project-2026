...
Многоэтапная сборка (multi-stage) обеспечивает минимальный размер образа (~150 МБ) @docker-docs.
Развёртывание в промышленной среде (production) выполняется в кластере Kubernetes @kubernetes-docs с использованием Helm-чартов @helm-docs.
...
#figure(
  table(
    ...
    [Kubernetes], [1.25+],
    [Helm], [3.10+],
    [ОЗУ (в режиме выполнения)], [1 ГБ],
    [Диск (база данных)], [10 ГБ],
  ),
